...
class Decoding(ABC):
    ...
    def parallel_speculative_decoding(self, prefix):
        ...
        # Synchronize prefix across processes at the start
        if self.accelerator.is_main_process:
            prefix_to_broadcast = prefix
        else:
            prefix_to_broadcast = None
        prefix = self.accelerator.broadcast(prefix_to_broadcast)
        ...
        while prefix.shape[1] < max_tokens:
            ...
            if self.accelerator.is_main_process:
                ...
                # Create placeholder tensor for target probability
                # Using the same shape as a draft prob for compatibility
                placeholder_target = torch.zeros_like(draft_probs[0])
            else:
                # Non-main process: Generate target probability
                x = model.generate(input_ids, 1)
                target_prob = model._prob_history[:, prefix_len - self.args.gamma - 1:prefix_len, :self.vocab_size].to(torch.float32)
            ...
            if self.accelerator.is_main_process:
                # Main process sends placeholder, receives target_prob
                gathered_probs = self.accelerator.gather(placeholder_target)
                # Extract target probability from the second process (index 1)
                target_prob = gathered_probs[1]  # Assuming world size of 2
            else:
                # Non-main process sends target_prob
                self.accelerator.gather(target_prob)
            
            # Only do verification in main process
            best_prefix = prefix.clone()
            best_total_accepted = 0
            
            if self.accelerator.is_main_process:
                # Evaluate each draft model
                for draft_idx, draft_prob in enumerate(draft_probs):
                    # Get this draft's tokens
                    draft_ids = draft_tokens[draft_idx]
                    ...
                    if cur_mode:
                        ...
                        if r <= target_prob[:, -1, first_token] / draft_prob[:, -1, first_token]:
                            # Accept first token
                            first_token_reshaped = first_token.view(1, 1)  # Ensure correct shape
                            temp_prefix = torch.cat((input_ids, first_token_reshaped), dim=1)
                            total_accepted += 1
                            ...
                            if self.args.gamma > 1:
                                ...
                                if n == self.args.gamma:
                                    # All remaining tokens accepted
                                    remaining_tokens = draft_ids[:, 1:self.args.gamma]
                                    temp_prefix = torch.cat((temp_prefix, remaining_tokens), dim=1)
                                    total_accepted = self.args.gamma
                                else:
                                    # Some token rejected, sample new token
                                    t = sample(max_fn(target_prob[:, n, :] - draft_prob[:, n, :]))
                                    t_reshaped = t.view(1, 1)  # Ensure correct shape
                                    temp_prefix = torch.cat((temp_prefix, t_reshaped), dim=1)
                                    total_accepted = n + 1  # Count first token + accepted tokens
                            ...
                            t = sample(max_fn(target_prob[:, -1, :] - draft_prob[:, -1, :]))
                            t_reshaped = t.view(1, 1)  # Ensure correct shape
                            temp_prefix = torch.cat((input_ids, t_reshaped), dim=1)
                            total_accepted = 1  # Just the sampled token
