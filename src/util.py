...
def parse_arguments():
    ...
    model_zoo(args)
    assert args.num_samples_per_task == len(args.draft_models), f"n should be equal to the length of draft_models, but got n={args.num_samples_per_task} and draft models={len(args.draft_models)}"
    return args
